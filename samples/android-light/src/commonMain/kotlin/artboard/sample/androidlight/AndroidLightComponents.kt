package artboard.sample.androidlight
...
import artboard.sample.androidlight.resources.Res
import artboard.sample.androidlight.resources.receipt_total
import org.jetbrains.compose.resources.stringResource

/**
 * Sample UI for the snapshot ("light mode") consumer contract on Android.
 *
 * This module declares **only** an `android` target — no `wasmJs`, no `jvm`. It exists
 * to prove that Artboard binds to a target the consumer already has, and that previews
 * whose dependency graph is not Wasm-safe still get a gallery.
 */
...
@Composable
private fun AndroidSampleTheme(content: @Composable () -> Unit) {
    MaterialTheme(
 ...
 * the Robolectric renderer's locale axis is verified end to end.
 */
@Preview(name = "Receipt · itemized", group = "Cards")
@Composable
fun ReceiptCardPreview() = AndroidSampleTheme {
    ReceiptCard(
...
@Composable
fun ReceiptCardEmptyPreview() = AndroidSampleTheme {
    ReceiptCard(
        merchant = "Corner Roasters",
...
@Composable
fun WideReceiptPreview() = AndroidSampleTheme {
    ReceiptCard(
...
@Composable
fun LedgerScreenPreview() = AndroidSampleTheme {
    LedgerScreen(
        title = "Ledger",
